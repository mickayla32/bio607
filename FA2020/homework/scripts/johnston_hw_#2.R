...
str(penguins)
# From str I can see that there are NAs in the columns bill_length_mm, bill_depth_mm, flipper_length_mm, body_mass_g, and sex
summary(penguins)
# From summary I can see how many NAs per column there are-- 2 in bill length, 2 in bill depth, 2 in flipper length, 2 in body mass, and 11 in sex
...
#   2E Make a species-island column in penguins using paste(). 
  # This is an awesome function that takes multiple strings, 
    #and slams them together using the argument sep = to define how the string should be combined. 
    # Try out paste("Hello", "Goodbye", sep = "! ") to see how it works. Have the two be separated by _.
...
penguins <- penguins %>%
  ...
  mutate(avg_flip = mean(flipper_length_mm, na.rm = TRUE),
         avg_body = mean(body_mass_g, na.rm = TRUE))

plot(avg_body ~ avg_flip, data =penguins)
# Penguins with high average flipper lengths also have high average body masses
...
#   4c. Use map_df() to create a data frame with the mean and sd of different sample sizes using 
# the first 5 through 100 values (so, n = 5:100 - smallest sample size will have the values 1-5). 
...
# 4d. Plot the relationship between sample size and SD and sample size versus SE of the mean. 
  # What difference do you see and why? Note, you’ll need to create a column for SE here!
...
# The standard deviation is higher with higher overall, but pretty consistent from sample sizes 40-100, but the standard error appears to decrease with higher sample size
...
# Average properties of bills by species, sex, and island

penguins <- penguins %>%
  group_by(species) %>%
  mutate(avg_bill_length_spec = mean(bill_length_mm, na.rm = TRUE),
         avg_bill_depth_spec = mean(bill_depth_mm, na.rm = TRUE)) %>%
  ungroup()

penguins <- penguins %>%
  group_by(sex) %>%
  mutate(avg_bill_length_sex = mean(bill_length_mm, na.rm = TRUE),
         avg_bill_depth_sex = mean(bill_depth_mm, na.rm = TRUE)) %>%
  ungroup()

penguins <- penguins %>%
  group_by(island) %>%
  mutate(avg_bill_length_isl = mean(bill_length_mm, na.rm = TRUE),
         avg_bill_depth_isl = mean(bill_depth_mm, na.rm = TRUE)) %>%
  ungroup()


# Select the desired columns for table
penguins_table <- penguins %>%
  select(species, island, sex, avg_bill_length_spec, avg_bill_depth_spec,
...
#tabledf_2 <- tidyr::pivot_wider(data= tabledf_1,
#                   id_cols = 1:3)
